...
//    self.navigationItem.rightBarButtonItem = bar;
    ...
    myTableView = [[UITableView alloc]initWithFrame:CGRectMake(0, height, SCREEN_WIDTH, SCREEN_HEIGHT - height) style:UITableViewStylePlain];
...
- (void)lodXL{
    ...
    [session POST:KURLNSString2(@"servlet", @"batch", @"batchshoppingcart",@"list") parameters:dic constructingBodyWithBlock:nil progress:^(NSProgress * _Nonnull uploadProgress) {
...
-(void)countPrice
{
    ...
    for (PL_03_Model *model in selectGoods) {
        double price = [model.unitPrice doubleValue];
...
-(void)selectAllBtnClick:(UIButton*)sender
{
    ...
    if (isSelect) {
        for (PL_03_Model *model in dataArray) {
...
- (void)lodOrderYULan:(NSMutableArray *)ids {
    ...
    if (selectGoods.count != 0) {
        PL_03_ViewController *pl03 = [[PL_03_ViewController alloc]init];
...
- (void)goShoppingBtnClick{
    PL_02_ViewController *pl02 = [[PL_02_ViewController alloc]init];
...
- (void)clickgodeleate {
    ...
    NSMutableArray *arr = [NSMutableArray arrayWithCapacity:1];
    for (PL_03_Model *model in deleateArr) {
        [arr addObject:model.id];
    }
...
- (void)lodDelegateSelectsGoods:(NSString *)idstr{
    ...
    [session POST:KURLNSString2(@"servlet", @"batch", @"batchshoppingcart",@"delete") parameters:dic constructingBodyWithBlock:nil progress:^(NSProgress * _Nonnull uploadProgress) {
    } success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
//        NSDictionary *dic = [Manager returndictiondata:responseObject];
//        NSLog(@"delete+++%@",dic);
        [myTableView reloadData];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.numAddBlock =^(){
        ...
        if (count < [[Manager sharedManager].gerenMax integerValue]) {
            count++;
        }
        NSString *numStr = [NSString stringWithFormat:@"%ld",(long)count];
        
        PL_03_Model *model = [dataArray objectAtIndex:indexPath.row];
    ...
    cell.numCutBlock =^(){
        ...
        NSInteger count = [weakCell.numberLabel.text integerValue];
        if (count > [[Manager sharedManager].gerenMin integerValue]) {
            count--;
        }
...
- (void)lodChangeGoodsNumber:(NSString *)amount ids:(NSString *)ids indexpath:(NSIndexPath *)indexpath{
    ...
    [session POST:KURLNSString2(@"servlet", @"batch", @"batchshoppingcart",@"update") parameters:dic constructingBodyWithBlock:nil progress:^(NSProgress * _Nonnull uploadProgress) {
    } success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        NSDictionary *dic = [Manager returndictiondata:responseObject];
