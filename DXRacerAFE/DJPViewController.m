...
#import "DJP_KPXX_ViewController.h"

@interface DJPViewController ()<UITableViewDelegate,UITableViewDataSource>
{
    NSInteger totalnum;
    NSInteger page;
}
...
@implementation DJPViewController
...
- (void)viewDidLoad {
    ...
    [self.tableview registerNib:[UINib nibWithNibName:@"DJP_Cell" bundle:nil] forCellReuseIdentifier:@"cell"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    DJP_Cell *cell = [tableView dequeueReusableCellWithIdentifier:identifierCell];
    if (cell == nil) {
        cell = [[DJP_Cell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:identifierCell];
    }
    ...
    cell.lab2.text = model.invoiceType;
    cell.lab3.text = model.title;
    cell.lab4.text = model.invoiceCode;
    cell.lab5.text = model.invoiceNumber;
    
    
    
    if (model.receivePerson == nil) {
        cell.lab6.text = @"未填写收票地址";
        cell.lab6.textColor = [UIColor redColor];
    }else{
        cell.lab6.text = model.receivePerson;
    }
    
    
    
    cell.lab7.text = model.phone;
    cell.lab8.text = model.receiveAddress;
...
- (void)clickselected:(UIButton *)sender{
    DJP_Cell *cell = (DJP_Cell *)[[sender.superview superview] superview];
    NSIndexPath *indexpath = [self.tableview indexPathForCell:cell];
    WDFPModel *model = [self.dataArray objectAtIndex:indexpath.row];
    
    DJP_KPXX_ViewController *addr = [[DJP_KPXX_ViewController alloc]init];
    addr.navigationItem.title = @"填写开票信息";
    addr.strid = model.id;
    
    addr.str1 = model.title;
    addr.str2 = [Manager jinegeshi:model.totalFee];
    addr.str3 = model.invoiceCode;
    addr.str4 = model.invoiceNumber;
...
- (void)clickdetails:(UIButton *)sender{
    ...
    details.str9  = model.receiveAddress;
