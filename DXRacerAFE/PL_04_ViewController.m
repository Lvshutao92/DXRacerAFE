...
@interface PL_04_ViewController ()<UITableViewDelegate,UITableViewDataSource,UISearchBarDelegate,UITextFieldDelegate>
{
    NSInteger totalnum;
    NSInteger page;
    CGFloat height;
...
@implementation PL_04_ViewController
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    // 创建Attributed
    NSMutableAttributedString *noteStr = [[NSMutableAttributedString alloc] initWithString: [NSString stringWithFormat:@"%@: %@%@",NSLocalizedString(@"a2", nil),model.model2.field1,[Manager jinegeshi111:model.productTotalFee]]];
    // 需要改变的区间
    ...
    if (model.planDeliveryDate != nil) {
        cell.lab5.text = [NSString stringWithFormat:@"%@: %@",NSLocalizedString(@"a6", nil),[Manager TimeCuoToTime:model.planDeliveryDate]];
    }else{
        cell.lab5.text = [NSString stringWithFormat:@"%@: %@",NSLocalizedString(@"a6", nil),@"-"];
    }
...
//    NSLog(@"%@",model.orderStatus);
    ...
    if (model.customOrderNo ==nil) {
        cell.lab6.text = [NSString stringWithFormat:@"%@：%@",NSLocalizedString(@"g13", nil),@""];
    }else{
        cell.lab6.text = [NSString stringWithFormat:@"%@：%@",NSLocalizedString(@"g13", nil),model.customOrderNo];
    }
    if (model.field7 ==nil) {
        cell.lab7.text = [NSString stringWithFormat:@"%@：%@",NSLocalizedString(@"g14", nil),@"-"];
    }else{
        cell.lab7.text = [NSString stringWithFormat:@"%@：%@",NSLocalizedString(@"g14", nil),model.field7];
    }
...
- (void)clickbtn:(UIButton *)sender{
    OrderListCell *cell = (OrderListCell *)[[[sender superview] superview] superview];
...
//        NSLog(@"%@",[dic objectForKey:@"message"]);
        if ([[dic objectForKey:@"code"]isEqualToString:@"200"]) {
            UIAlertController *alert = [UIAlertController alertControllerWithTitle:NSLocalizedString(@"a24", nil) message:NSLocalizedString(@"x9", nil) preferredStyle:1];
            ...
            UIAlertController *alert = [UIAlertController alertControllerWithTitle:NSLocalizedString(@"x9", nil) message:[dic objectForKey:@"message"] preferredStyle:1];
...
- (void)loddeList{
    ...
    [session POST:KURLNSString3(@"servlet", @"order", @"dealer", @"batch",@"list") parameters:dic constructingBodyWithBlock:nil progress:^(NSProgress * _Nonnull uploadProgress) {
    } success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        NSDictionary *dic = [Manager returndictiondata:responseObject];
