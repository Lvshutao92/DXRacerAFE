...
#import "JC_4_Cell.h"
#import "JC_14_Model.h"
@interface JC_14_ViewController ()<UITableViewDelegate,UITableViewDataSource>
{
    NSInteger totalnum;
    NSInteger page;
}
...
@implementation JC_14_ViewController
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    JC_14_Model *model = [self.dataArray objectAtIndex:indexPath.row];
    
    cell.lab1.text = model.nameEn;
    cell.lab2.text = model.nameCn;
...
- (void)lodAdd:(NSString *)str1 str2:(NSString *)str2{
    ...
    [session POST:KURLNSString2(@"servlet", @"config", @"configduration", @"add") parameters:dic constructingBodyWithBlock:nil progress:^(NSProgress * _Nonnull uploadProgress) {
...
- (void)lodEdit:(NSString *)str1 str2:(NSString *)str2 idStr:(NSString *)idStr IndexPath:(NSIndexPath *)IndexPath{
    ...
    [session POST:KURLNSString2(@"servlet", @"config", @"configduration", @"update") parameters:dic constructingBodyWithBlock:nil progress:^(NSProgress * _Nonnull uploadProgress) {
    } success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        NSDictionary *dic = [Manager returndictiondata:responseObject];
        if ([[dic objectForKey:@"result_code"]isEqualToString:@"success"]) {
...
- (nullable NSArray<UITableViewRowAction *> *)tableView:(UITableView *)tableView editActionsForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    UITableViewRowAction *suer = [UITableViewRowAction rowActionWithStyle:UITableViewRowActionStyleDestructive title:@"编辑" handler:^(UITableViewRowAction * _Nonnull sure, NSIndexPath * _Nonnull indexPath) {
        ...
        [alertController addAction:[UIAlertAction actionWithTitle:@"取消" style:UIAlertActionStyleCancel handler:nil]];
        [alertController addTextFieldWithConfigurationHandler:^(UITextField * _Nonnull textField) {
            textField.text = model.nameCn;
        }];
        [alertController addTextFieldWithConfigurationHandler:^(UITextField * _Nonnull textField) {
            textField.text = model.nameEn;
        }];
...
- (void)lodDelete:(NSString *)idStr IndexPath:(NSIndexPath *)IndexPath{
    ...
    [session POST:KURLNSString2(@"servlet", @"config", @"configduration", @"delete") parameters:dic constructingBodyWithBlock:nil progress:^(NSProgress * _Nonnull uploadProgress) {
    } success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        NSDictionary *dic = [Manager returndictiondata:responseObject];
...
- (void)loddeList{
    ...
    [session POST:KURLNSString2(@"servlet", @"config", @"configduration", @"list") parameters:dic constructingBodyWithBlock:nil progress:^(NSProgress * _Nonnull uploadProgress) {
    } success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        NSDictionary *dic = [Manager returndictiondata:responseObject];
...
//        NSLog(@"---%@",dic);
        ...
        for (NSDictionary *temp in array) {
            JC_14_Model *model = [JC_14_Model mj_objectWithKeyValues:temp];
