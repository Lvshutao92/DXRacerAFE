...
@interface XH_03_ViewController ()<UITableViewDelegate,UITableViewDataSource>
{
...
@implementation XH_03_ViewController
...
- (void)lodXL{
    ...
    [session POST:KURLNSString2(@"servlet", @"trade", @"tradeshoppingcart",@"list") parameters:dic constructingBodyWithBlock:nil progress:^(NSProgress * _Nonnull uploadProgress) {
...
//        NSLog(@"cart+++%@",dic);
        ...
        for (NSDictionary *dict in arr) {
            XH03Model *model = [XH03Model mj_objectWithKeyValues:dict];
            
            tradeGoodsModel *model1 = [tradeGoodsModel mj_objectWithKeyValues:model.tradeGoods];
            model.model1 = model1;
            
            dealeGModel *model2 = [dealeGModel mj_objectWithKeyValues:model.dealerGoods];
            model.model2 = model2;
            
            tradeGoodsModel *model3 = [tradeGoodsModel mj_objectWithKeyValues:model2.dealerInfo];
            model2.dealerInfomodel = model3;
            
            configCurrency_Model *model4 = [configCurrency_Model mj_objectWithKeyValues:model3.configCurrency];
            model3.currmodel = model4;
...
-(void)countPrice
{
    ...
    for (XH03Model *model in selectGoods) {
...
-(void)selectAllBtnClick:(UIButton*)sender
{
    ...
    if (isSelect) {
        for (XH03Model *model in dataArray) {
...
- (void)lodOrderYULan:(NSMutableArray *)ids {
    ...
    if (selectGoods.count != 0) {
        OR_YL_ViewController *pl03 = [[OR_YL_ViewController alloc]init];
...
- (void)goShoppingBtnClick{
    XH_02_ViewController *pl02 = [[XH_02_ViewController alloc]init];
...
- (void)clickgodeleate {
    ...
    NSMutableArray *arr = [NSMutableArray arrayWithCapacity:1];
    for (XH03Model *model in deleateArr) {
        [arr addObject:model.id];
    }
...
{
    XH03Cell *cell = [tableView dequeueReusableCellWithIdentifier:@"shoopingcell"];
    if (!cell) {
        cell = [[XH03Cell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"shoopingcell"];
    ...
    __block XH03Cell *weakCell = cell;
    cell.numAddBlock =^(){
        ...
        XH03Model *model = [dataArray objectAtIndex:indexPath.row];
...
//    NSLog(@"---%@",dic);
    [session POST:KURLNSString2(@"servlet",@"trade", @"tradeshoppingcart",@"update") parameters:dic constructingBodyWithBlock:nil progress:^(NSProgress * _Nonnull uploadProgress) {
    } success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        NSDictionary *dic = [Manager returndictiondata:responseObject];
//        NSLog(@"+++%@",dic);
        ...
        //        [dataArray removeAllObjects];
        //        for (NSDictionary *dict in arr) {
        //            PL_03_Model *model = [PL_03_Model mj_objectWithKeyValues:dict];
        //
        //            CartModel *model1 = [CartModel mj_objectWithKeyValues:model.cart];
        //            model.cartmodel = model1;
        //
        //            [dataArray addObject:model];
        //        }
...
- (void)lodDelegateSelectsGoods:(NSString *)idstr{
    ...
    [session POST:KURLNSString2(@"servlet", @"trade", @"tradeshoppingcart",@"delete") parameters:dic constructingBodyWithBlock:nil progress:^(NSProgress * _Nonnull uploadProgress) {
    } success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        //        NSDictionary *dic = [Manager returndictiondata:responseObject];
        //        NSLog(@"delete+++%@",dic);
        [myTableView reloadData];
