...
#import "JC_4_Cell.h"
#import "JC_10_Model.h"
@interface JC_10_ViewController ()<UITableViewDelegate,UITableViewDataSource>
{
    NSInteger totalnum;
    NSInteger page;
}
...
@implementation JC_10_ViewController
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    JC_10_Model *model = [self.dataArray objectAtIndex:indexPath.row];
    
    cell.laba.text = @"证书类型英文名";
    cell.laba.font = [UIFont systemFontOfSize:13];
    cell.labb.font = [UIFont systemFontOfSize:13];
    cell.lab1.font = [UIFont systemFontOfSize:13];
    cell.lab2.font = [UIFont systemFontOfSize:13];
    cell.labb.text = @"证书类型中文名";
    
    cell.lab1.text = model.certificateNameEn;
    cell.lab2.text = model.certificateNameCn;
...
- (void)lodAdd:(NSString *)str1 str2:(NSString *)str2{
    ...
    dic = @{@"businessId":[Manager redingwenjianming:@"businessId.text"],
            @"certificateNameCn":str1,
            @"certificateNameEn":str2,
            };
    [session POST:KURLNSString2(@"servlet", @"config", @"configcertificate", @"add") parameters:dic constructingBodyWithBlock:nil progress:^(NSProgress * _Nonnull uploadProgress) {
...
- (void)lodEdit:(NSString *)str1 str2:(NSString *)str2 idStr:(NSString *)idStr IndexPath:(NSIndexPath *)IndexPath{
    ...
    [session POST:KURLNSString2(@"servlet", @"config", @"configcertificate", @"update") parameters:dic constructingBodyWithBlock:nil progress:^(NSProgress * _Nonnull uploadProgress) {
    } success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        NSDictionary *dic = [Manager returndictiondata:responseObject];
        if ([[dic objectForKey:@"result_code"]isEqualToString:@"success"]) {
...
- (nullable NSArray<UITableViewRowAction *> *)tableView:(UITableView *)tableView editActionsForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    UITableViewRowAction *suer = [UITableViewRowAction rowActionWithStyle:UITableViewRowActionStyleDestructive title:@"编辑" handler:^(UITableViewRowAction * _Nonnull sure, NSIndexPath * _Nonnull indexPath) {
        ...
        [alertController addAction:[UIAlertAction actionWithTitle:@"取消" style:UIAlertActionStyleCancel handler:nil]];
        [alertController addTextFieldWithConfigurationHandler:^(UITextField * _Nonnull textField) {
            textField.text = model.certificateNameCn;
        }];
        [alertController addTextFieldWithConfigurationHandler:^(UITextField * _Nonnull textField) {
            textField.text = model.certificateNameEn;
        }];
...
- (void)lodDelete:(NSString *)idStr IndexPath:(NSIndexPath *)IndexPath{
    ...
    [session POST:KURLNSString2(@"servlet", @"config", @"configcertificate", @"delete") parameters:dic constructingBodyWithBlock:nil progress:^(NSProgress * _Nonnull uploadProgress) {
    } success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        NSDictionary *dic = [Manager returndictiondata:responseObject];
...
- (void)loddeList{
    ...
    [session POST:KURLNSString2(@"servlet", @"config", @"configcertificate", @"list") parameters:dic constructingBodyWithBlock:nil progress:^(NSProgress * _Nonnull uploadProgress) {
    } success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        NSDictionary *dic = [Manager returndictiondata:responseObject];
...
//        NSLog(@"---%@",dic);
        ...
        for (NSDictionary *temp in array) {
            JC_10_Model *model = [JC_10_Model mj_objectWithKeyValues:temp];
