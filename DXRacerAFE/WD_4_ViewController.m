...
@interface WD_4_ViewController ()<UITableViewDelegate,UITableViewDataSource>
{
    NSInteger totalnum;
    NSInteger page;
}
...
@implementation WD_4_ViewController
...
- (void)viewDidLoad {
    ...
    [self.tableview registerNib:[UINib nibWithNibName:@"WD_4_Cell" bundle:nil] forCellReuseIdentifier:@"cell"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    WD_4_Cell *cell = [tableView dequeueReusableCellWithIdentifier:reuseID];
    if (cell == nil) {
        cell = [[WD_4_Cell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:reuseID];
    }
    ...
    WD_4_model *model = [self.dataArray objectAtIndex:indexPath.row];
...
- (void)clickedit:(UIButton *)sender{
    WD_4_Cell *cell = (WD_4_Cell *)[[sender.superview superview] superview];
    NSIndexPath *indexpath = [self.tableview indexPathForCell:cell];
    WD_4_model *model = [self.dataArray objectAtIndex:indexpath.row];
    Person_add_edit_ViewController *edit = [[Person_add_edit_ViewController alloc]init];
    ...
    edit.str2 = model.model1.chineseName;
    edit.str2id = model.model1.id;
    
    edit.str3 = model.qq;
    edit.str4 = model.wechat;
    edit.str5 = model.telephone;
    edit.str6 = model.email;
   
    [self.navigationController pushViewController:edit animated:YES];
}
- (void)clickadd{
    Person_add_edit_ViewController *add = [[Person_add_edit_ViewController alloc]init];
...
- (void)clickdele:(UIButton *)sender{
    ...
    [session POST:KURLNSString3(@"servlet", @"dealer", @"mine",@"dealercontact",@"delete") parameters:dic constructingBodyWithBlock:nil progress:^(NSProgress * _Nonnull uploadProgress) {
...
- (void)loddeList{
    ...
    [session POST:KURLNSString3(@"servlet", @"dealer", @"mine", @"dealercontact",@"list") parameters:dic constructingBodyWithBlock:nil progress:^(NSProgress * _Nonnull uploadProgress) {
    } success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        NSDictionary *dic = [Manager returndictiondata:responseObject];
        ...
        for (NSDictionary *dic in arr) {
            WD_4_model *model = [WD_4_model mj_objectWithKeyValues:dic];
