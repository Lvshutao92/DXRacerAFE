...
#import "JC_4_Cell.h"
#import "JC_11_Model.h"

@interface JC_11_ViewController ()<UITableViewDelegate,UITableViewDataSource>
{
    NSInteger totalnum;
    NSInteger page;
}
...
@implementation JC_11_ViewController
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    JC_11_Model *model = [self.dataArray objectAtIndex:indexPath.row];
    cell.laba.text = @"产地证英文名";
    cell.labb.text = @"产地证中文名";
    
    cell.lab1.text = model.certificateOriginNameEn;
    cell.lab2.text = model.certificateOriginNameCn;
...
- (void)lodAdd:(NSString *)str1 str2:(NSString *)str2{
    ...
    dic = @{@"businessId":[Manager redingwenjianming:@"businessId.text"],
            @"certificateOriginNameCn":str1,
            @"certificateOriginNameEn":str2,
            };
    [session POST:KURLNSString2(@"servlet", @"config", @"configcertificateorigin", @"add") parameters:dic constructingBodyWithBlock:nil progress:^(NSProgress * _Nonnull uploadProgress) {
...
- (void)lodEdit:(NSString *)str1 str2:(NSString *)str2 idStr:(NSString *)idStr IndexPath:(NSIndexPath *)IndexPath{
    ...
    [session POST:KURLNSString2(@"servlet", @"config", @"configcertificateorigin", @"update") parameters:dic constructingBodyWithBlock:nil progress:^(NSProgress * _Nonnull uploadProgress) {
    } success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        NSDictionary *dic = [Manager returndictiondata:responseObject];
        if ([[dic objectForKey:@"result_code"]isEqualToString:@"success"]) {
...
- (nullable NSArray<UITableViewRowAction *> *)tableView:(UITableView *)tableView editActionsForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    UITableViewRowAction *suer = [UITableViewRowAction rowActionWithStyle:UITableViewRowActionStyleDestructive title:@"编辑" handler:^(UITableViewRowAction * _Nonnull sure, NSIndexPath * _Nonnull indexPath) {
        ...
        [alertController addAction:[UIAlertAction actionWithTitle:@"取消" style:UIAlertActionStyleCancel handler:nil]];
        [alertController addTextFieldWithConfigurationHandler:^(UITextField * _Nonnull textField) {
            textField.text = model.certificateOriginNameCn;
        }];
        [alertController addTextFieldWithConfigurationHandler:^(UITextField * _Nonnull textField) {
            textField.text = model.certificateOriginNameEn;
        }];
...
- (void)lodDelete:(NSString *)idStr IndexPath:(NSIndexPath *)IndexPath{
    ...
    [session POST:KURLNSString2(@"servlet", @"config", @"configcertificateorigin", @"delete") parameters:dic constructingBodyWithBlock:nil progress:^(NSProgress * _Nonnull uploadProgress) {
    } success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        NSDictionary *dic = [Manager returndictiondata:responseObject];
...
- (void)loddeList{
    ...
    [session POST:KURLNSString2(@"servlet", @"config", @"configcertificateorigin", @"list") parameters:dic constructingBodyWithBlock:nil progress:^(NSProgress * _Nonnull uploadProgress) {
    } success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        NSDictionary *dic = [Manager returndictiondata:responseObject];
...
//                NSLog(@"---%@",dic);
        ...
        for (NSDictionary *temp in array) {
            JC_11_Model *model = [JC_11_Model mj_objectWithKeyValues:temp];
