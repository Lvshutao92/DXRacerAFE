...
- (void)viewDidLoad {
    [super viewDidLoad];
    self.navigationItem.title = NSLocalizedString(@"btn2", nil);
    UIButton *btn = [Manager returnButton];
    [btn addTarget:self action:@selector(back) forControlEvents:UIControlEventTouchUpInside];
    UIBarButtonItem *bar = [[UIBarButtonItem alloc]initWithCustomView:btn];
    self.navigationItem.leftBarButtonItem = bar;
//    self.arr = [@[NSLocalizedString(@"cp1", nil),NSLocalizedString(@"cp2", nil),@"售后部件"]mutableCopy];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if ([cell.textLabel.text isEqualToString:NSLocalizedString(@"cp1", nil)]) {
        CP_1_ViewController *jxs1 = [[CP_1_ViewController alloc]init];
        jxs1.navigationItem.title = NSLocalizedString(@"cp1", nil);
        [self.navigationController pushViewController:jxs1 animated:YES];
    }
    else if ([cell.textLabel.text isEqualToString:NSLocalizedString(@"cp2", nil)]) {
        CP_2_ViewController *jxs1 = [[CP_2_ViewController alloc]init];
        jxs1.navigationItem.title = NSLocalizedString(@"cp2", nil);
        [self.navigationController pushViewController:jxs1 animated:YES];
    }
    else if ([cell.textLabel.text isEqualToString:@"售后部件"]) {
        CP_3_ViewController *jxs1 = [[CP_3_ViewController alloc]init];
        jxs1.navigationItem.title = @"售后部件";
        [self.navigationController pushViewController:jxs1 animated:YES];
    }
}
/*
