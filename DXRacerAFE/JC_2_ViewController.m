...
@interface JC_2_ViewController ()<UITableViewDelegate,UITableViewDataSource>
{
    NSInteger totalnum;
    NSInteger page;
}
...
@implementation JC_2_ViewController
...
- (void)viewDidLoad {
    ...
    [self.tableview registerNib:[UINib nibWithNibName:@"JC_2_Cell" bundle:nil] forCellReuseIdentifier:@"cell"];
...
- (void)lodAdd:(NSString *)str1 str2:(NSString *)str2{
    ...
    [session POST:KURLNSString2(@"servlet", @"config", @"configlogistic", @"add") parameters:dic constructingBodyWithBlock:nil progress:^(NSProgress * _Nonnull uploadProgress) {
...
- (void)clickadd{
    ...
    [alertController addTextFieldWithConfigurationHandler:^(UITextField * _Nonnull textField) {
        textField.placeholder = @"承运商编码";
    }];
    [alertController addTextFieldWithConfigurationHandler:^(UITextField * _Nonnull textField) {
        textField.placeholder = @"承运商名称";
    }];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    JC_2_Cell *cell = [tableView dequeueReusableCellWithIdentifier:@"cell" forIndexPath:indexPath];
    cell.selectionStyle = UITableViewCellSelectionStyleNone;
    JC_2_Model *model = [self.dataArray objectAtIndex:indexPath.row];
    
    cell.lab1.text = model.logisticCode;
    cell.lab2.text = model.logisticName;
...
- (void)lodEdit:(NSString *)str1 str2:(NSString *)str2 idstr:(NSString *)idstr{
    ...
    [session POST:KURLNSString2(@"servlet", @"config", @"configlogistic", @"update") parameters:dic constructingBodyWithBlock:nil progress:^(NSProgress * _Nonnull uploadProgress) {
    } success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        NSDictionary *dic = [Manager returndictiondata:responseObject];
        if ([[dic objectForKey:@"result_code"]isEqualToString:@"success"]) {
...
- (nullable NSArray<UITableViewRowAction *> *)tableView:(UITableView *)tableView editActionsForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    UITableViewRowAction *suer = [UITableViewRowAction rowActionWithStyle:UITableViewRowActionStyleDestructive title:@"编辑" handler:^(UITableViewRowAction * _Nonnull sure, NSIndexPath * _Nonnull indexPath) {
        ...
        [alertController addAction:[UIAlertAction actionWithTitle:@"取消" style:UIAlertActionStyleCancel handler:nil]];
        [alertController addTextFieldWithConfigurationHandler:^(UITextField * _Nonnull textField) {
            textField.text = model.logisticCode;
        }];
        [alertController addTextFieldWithConfigurationHandler:^(UITextField * _Nonnull textField) {
            textField.text = model.logisticName;
        }];
        [self presentViewController:alertController animated:true completion:nil];
...
- (void)lodDelete:(NSString *)idStr IndexPath:(NSIndexPath *)IndexPath{
    ...
    [session POST:KURLNSString2(@"servlet", @"config", @"configlogistic", @"delete") parameters:dic constructingBodyWithBlock:nil progress:^(NSProgress * _Nonnull uploadProgress) {
    } success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        NSDictionary *dic = [Manager returndictiondata:responseObject];
        if ([[dic objectForKey:@"result_code"]isEqualToString:@"success"]) {
...
- (void)loddeList{
    ...
    [session POST:KURLNSString2(@"servlet", @"config", @"configlogistic", @"list") parameters:dic constructingBodyWithBlock:nil progress:^(NSProgress * _Nonnull uploadProgress) {
    } success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        NSDictionary *dic = [Manager returndictiondata:responseObject];
        ...
        for (NSDictionary *dic in arr) {
            JC_2_Model *model = [JC_2_Model mj_objectWithKeyValues:dic];
