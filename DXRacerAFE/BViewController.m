...
- (void)lodXL{
    ...
    [session POST:KURLNSString3(@"servlet", @"retainage", @"dealer", @"paylist",@"list") parameters:dic constructingBodyWithBlock:nil progress:^(NSProgress * _Nonnull uploadProgress) {
...
- (void)lodOrderYULan:(NSString *)ids orderids:(NSString *)orderids{
    ...
    UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"温馨提示" message:[NSString stringWithFormat:@"您需要支付的尾款合计为:%.02fRMB，确认付款吗？",money] preferredStyle:1];
    ...
    UIAlertAction *action = [UIAlertAction actionWithTitle:@"确定" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        ...
        dic = @{@"orderIds":orderids,
                ...
                @"paymentType":@"Retainage",
                };
        [session POST:KURLNSString3(@"servlet", @"retainage", @"dealer", @"paylist",@"generate") parameters:dic constructingBodyWithBlock:nil progress:^(NSProgress * _Nonnull uploadProgress) {
        } success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
            NSDictionary *dic = [Manager returndictiondata:responseObject];
//            NSLog(@"%@",dic);
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    BCell *cell = [tableView dequeueReusableCellWithIdentifier:identifierCell];
    if (cell == nil) {
        cell = [[BCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:identifierCell];
    }
