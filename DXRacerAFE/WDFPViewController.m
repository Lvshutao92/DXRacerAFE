...
@interface WDFPViewController ()<UITableViewDelegate,UITableViewDataSource>
{
    NSInteger totalnum;
    NSInteger page;
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if ([model.invoiceStatus isEqualToString:@"apply"]) {
        ...
        [cell.selectbtn addTarget:self action:@selector(clickselected:) forControlEvents:UIControlEventTouchUpInside];
        cell.selectbtnwidth.constant = 95;
...
- (void)clickselected:(UIButton *)sender{
    ...
    FP_addr_ViewController *addr = [[FP_addr_ViewController alloc]init];
    addr.navigationItem.title = @"选择收票地址";
    addr.strid = model.id;
...
- (void)loddeList{
    ...
    [session POST:KURLNSString2(@"servlet", @"invoice",@"dealer",@"list")  parameters:dic constructingBodyWithBlock:nil progress:^(NSProgress * _Nonnull uploadProgress) {
