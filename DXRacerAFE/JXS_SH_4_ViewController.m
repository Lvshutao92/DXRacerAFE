...
@interface JXS_SH_4_ViewController ()<UITableViewDelegate,UITableViewDataSource,UISearchBarDelegate,UITextFieldDelegate>
{
    NSInteger totalnum;
    NSInteger page;
...
@implementation JXS_SH_4_ViewController
...
- (void)clickbtn:(UIButton *)sender{
    Piliang_orderlist_Cell *cell = (Piliang_orderlist_Cell *)[[[sender superview] superview] superview];
...
- (void)loddeList{
    ...
    [session POST:KURLNSString3(@"servlet", @"order", @"dealer", @"aftersale",@"list") parameters:dic constructingBodyWithBlock:nil progress:^(NSProgress * _Nonnull uploadProgress) {
    } success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        NSDictionary *dic = [Manager returndictiondata:responseObject];
