...
- (void)setMenuTitles:(NSArray *)titlesArr rowHeight:(CGFloat)rowHeight{
    ...
    _tableView.layer.borderColor   = [UIColor lightTextColor].CGColor;
...
- (void)showDropDown{   // 显示下拉列表
    ...
    [UIView animateWithDuration:AnimateTime animations:^{
        ...
        _listView.frame  = CGRectMake(VIEW_X(_listView), VIEW_Y(_listView), VIEW_WIDTH(_listView), (_rowHeight+10) *_titleArr.count);
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (cell == nil) {
        ...
        UIView * line = [[UIView alloc] initWithFrame:CGRectMake(0, (_rowHeight+10) -0.5, VIEW_WIDTH(cell), 0.5)];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([self.delegate respondsToSelector:@selector(dropdownMenu:selectedCellNumber:titleString:)]) {
        [self.delegate dropdownMenu:self selectedCellNumber:indexPath.row titleString:cell.textLabel.text]; // 回调代理
    }
