...
@interface DX_4_ViewController ()<UITableViewDelegate,UITableViewDataSource>
{
    NSInteger totalnum;
    NSInteger page;
}
...
@implementation DX_4_ViewController
...
- (void)viewDidLoad {
    ...
    [self.tableview registerNib:[UINib nibWithNibName:@"DX4Cell" bundle:nil] forCellReuseIdentifier:@"cell"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    DX4Cell *cell = [tableView dequeueReusableCellWithIdentifier:reuseID];
    if (cell == nil) {
        cell = [[DX4Cell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:reuseID];
    }
    cell.selectionStyle = UITableViewCellSelectionStyleNone;
    Dx4model *model = [self.dataArray objectAtIndex:indexPath.row];
...
- (void)clickedit:(UIButton *)sender{
    DX4Cell *cell = (DX4Cell *)[[sender.superview superview] superview];
    NSIndexPath *indexpath = [self.tableview indexPathForCell:cell];
    Dx4model *model = [self.dataArray objectAtIndex:indexpath.row];
    
    DX4_add_edit_ViewController *edit = [[DX4_add_edit_ViewController alloc]init];
...
- (void)clickdele:(UIButton *)sender{
    ...
    [session POST:KURLNSString2(@"servlet", @"config", @"configbank",@"delete") parameters:dic constructingBodyWithBlock:nil progress:^(NSProgress * _Nonnull uploadProgress) {
...
- (void)loddeList{
    ...
    [session POST:KURLNSString2(@"servlet", @"config", @"configbank",@"list") parameters:dic constructingBodyWithBlock:nil progress:^(NSProgress * _Nonnull uploadProgress) {
    } success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        NSDictionary *dic = [Manager returndictiondata:responseObject];
...
//               NSLog(@"dic = %@",dic);
        ...
        for (NSDictionary *dic in arr) {
            Dx4model *model = [Dx4model mj_objectWithKeyValues:dic];
            
            Dx4_con_model *model1 = [Dx4_con_model mj_objectWithKeyValues:model.configCurrency];
            model.model1 = model1;
